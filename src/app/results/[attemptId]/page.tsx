    ...
    return (
      <div className="min-h-screen bg-exam-bg flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-exam-border text-center max-w-sm w-full">
          ...
          <h2 className="font-bold text-lg text-slate-800">Calculating Negative Marks & Scorecard</h2>
          <p className="text-xs text-slate-500 mt-1">Applying -0.25 negative penalties...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-exam-bg flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-exam-border text-center max-w-md w-full">
          ...
          <h2 className="font-bold text-xl text-slate-800 mb-2">Scorecard Error</h2>
          <p className="text-sm text-slate-600 mb-6">{error || "Attempt data is unavailable"}</p>
          <Link
            href="/"
      ...
      <header className="bg-exam-primary text-white shadow-md">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="font-bold text-base sm:text-lg">Examination Scorecard & Analytics</h1>
          ...
          <div className="flex items-center space-x-3">
            <Link
              href="/"
              className="text-xs bg-white/10 hover:bg-white/20 text-white font-semibold px-3.5 py-2 rounded-lg transition"
            >
              Go to Lobby
            </Link>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 w-full flex-1 space-y-8">
        ...
        <div className="bg-white rounded-2xl shadow-md border border-exam-border p-6 sm:p-8 relative overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
            ...
            <div className="text-center md:text-left">
              ...
              <div className="flex items-baseline justify-center md:justify-start gap-2">
                <span className="text-5xl sm:text-6xl font-black text-exam-primary tracking-tight">
                  {score.netScore}
                </span>
                <span className="text-xl font-bold text-slate-400">/ 200</span>
              </div>
              <p className="text-xs text-slate-500 mt-2 font-medium">
                Formula: {score.correctCount} correct - ({score.wrongCount} wrong × 0.25 penalty)
              </p>
            </div>

            {/* Qualifying Status Badge */}
            <div className="flex flex-col items-center justify-center p-4 rounded-xl text-center border-2 border-dashed border-slate-200">
              {isQualifying ? (
                <div className="text-emerald-700 bg-emerald-50 w-full p-4 rounded-lg">
                  <div className="flex items-center justify-center gap-1.5 font-black text-base mb-1">
                    <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                    <span>QUALIFYING TARGET CLEARED</span>
                  </div>
                  <p className="text-xs text-emerald-800">
                    Net score {score.netScore} exceeds the 150/200 benchmark (75%).
                  </p>
                </div>
              ) : (
                <div className="text-rose-700 bg-rose-50 w-full p-4 rounded-lg">
                  <div className="flex items-center justify-center gap-1.5 font-black text-base mb-1">
                    <Target className="w-5 h-5 text-rose-600" />
                    <span>BELOW QUALIFYING TARGET</span>
                  </div>
                  <p className="text-xs text-rose-800">
                    Target is 150/200. You need {Number((150 - score.netScore).toFixed(2))} more net marks.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <span className="text-2xl font-black text-slate-800">{score.accuracyPercentage}%</span>
              </div>

              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-[11px] font-bold text-slate-500 uppercase block">Time Taken</span>
                <span className="text-2xl font-black text-slate-800 font-tabular">
                  {formatSeconds(attempt.timeTakenSeconds)}
                </span>
              </div>

              <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                <span className="text-[11px] font-bold text-emerald-700 uppercase block">Correct (+1)</span>
          ...
          <div className="mt-8 pt-6 border-t border-exam-border flex flex-wrap items-center justify-between gap-4">
            <div className="text-xs text-slate-500">
              Exam submitted on {new Date(attempt.submittedAt || Date.now()).toLocaleString()}
            </div>

            <div className="flex items-center space-x-3">
              <Link
                href="/"
                className="px-4 py-2 rounded-xl border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 text-xs sm:text-sm font-semibold transition"
              >
                Back to Lobby
              </Link>
              ...
              <button
                ...
                className="flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-exam-primary hover:bg-exam-primaryHover text-white text-xs sm:text-sm font-bold shadow-md transition disabled:opacity-50"
              >
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              <Award className="w-5 h-5 text-exam-primary" /> Section-wise Performance
            </h2>
                ...
                >
                  ...
                  <div className="space-y-1.5 text-xs">
                    ...
                    <div className="flex justify-between text-slate-600 pt-1 border-t border-slate-100">
                      <span>Accuracy:</span>
                      <span className="font-bold text-slate-800">{sec.accuracyPercentage}%</span>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-exam-border p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <FileText className="w-5 h-5 text-exam-primary" /> Full Paper Solution & Key Review
              </h2>
              <p className="text-xs text-slate-500">
                Review your responses against official answer keys and explanations.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 bg-slate-100 p-1 rounded-xl text-xs font-semibold">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition ${
                  filterType === "all" ? "bg-white text-exam-primary shadow-sm font-bold" : "text-slate-600"
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition ${
                  filterType === "wrong" ? "bg-white text-rose-700 shadow-sm font-bold" : "text-slate-600"
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition ${
                  filterType === "correct" ? "bg-white text-emerald-700 shadow-sm font-bold" : "text-slate-600"
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg transition ${
                  filterType === "unanswered" ? "bg-white text-slate-800 shadow-sm font-bold" : "text-slate-600"
                }`}
              >
          ...
          <div className="space-y-4">
            {filteredQuestions.map((item, idx) => {
              ...
              if (item.isCorrect) {
                ...
                statusBadge = (
                  <span className="text-xs font-bold text-rose-700 bg-rose-100 px-2.5 py-1 rounded-md border border-rose-200">
                    -0.25 Wrong
                  </span>
                );
              }
                ...
                >
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t border-slate-200/80 space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs sm:text-sm">
                        {(["a", "b", "c", "d"] as const).map((optKey) => {
                          ...
                          return (
                            ...
                            >
                              <span className="uppercase font-bold">({optKey})</span>
                      ...
                      {/* Explanation */}
                      {q.explanation && (
                        <div className="p-3.5 rounded-lg bg-blue-50/80 border border-blue-200 text-xs text-blue-900">
